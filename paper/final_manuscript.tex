...
0.19 s cold-start latency (19.3$\times$ speedup over PyTorch, 4.9$\times$ over MLX). \\
\midrule
...
78.41 tok/s CPU decode throughput in non-quantized Float16 with zero post-training quantization degradation. \\
\midrule
...
1.99$\times$ throughput speedup over PyTorch CPU with deterministic 13.99 ms inter-token latency. \\
\midrule
...
\textit{What VelocityAI fixes:} VelocityAI challenges this assumption by demonstrating that highly optimized ARM NEON vectorization using native half-precision (\texttt{Float16}) achieves 78.41 tokens/second on an Apple M2 CPU. By avoiding quantization entirely, VelocityAI preserves original model precision, eliminates post-training quantization degradation, and maintains model reasoning integrity.
...
VelocityAI eliminates intermediate user-space copies using POSIX virtual memory mapping (\texttt{mmap}). Upon first loading a model, the framework silently transcodes and caches an \texttt{optimized.velocityai.safetensors} file containing contiguous, pre-aligned Float16 buffers. On subsequent runs, this optimized artifact is mapped with \texttt{MAP\_SHARED}, creating virtual page table entries without eagerly allocating physical memory. Pointers for attention projections and feed-forward layers are computed directly via deterministic offset binding, driving the sub-second initialization times.
...
\begin{scope}[xshift=0cm]
    ...
    % Bars
    % PyTorch CPU: 39.34 -> 39.34/25 = 1.574
    \fill[torchgray!80, draw=black] (0.5,0) rectangle (1.5,1.574);
    \node[above, font=\scriptsize\bfseries] at (1.0, 1.574) {39.34};
    \node[below, align=center, font=\scriptsize] at (1.0, -0.15) {PyTorch\\\tiny(CPU, FP16)};
    
    % VelocityAI CPU: 78.41 -> 78.41/25 = 3.136
    \fill[velblue!90, draw=black, line width=1.1pt] (2.5,0) rectangle (3.5,3.136);
    \node[above, font=\scriptsize\bfseries, text=velblue] at (3.0, 3.136) {\textbf{78.41}};
    \node[below, align=center, font=\scriptsize\bfseries, text=velblue] at (3.0, -0.15) {VelocityAI\\\tiny(CPU, FP16)};
    
    % Apple MLX GPU: 77.84 -> 77.84/25 = 3.114
    \fill[mlxorange!85, draw=black] (4.5,0) rectangle (5.5,3.114);
    \node[above, font=\scriptsize\bfseries] at (5.0, 3.114) {77.84};
    ...
    % Annotations
    \draw[<->, >=stealth, dashed, thick, velblue] (1.0, 2.0) -- (3.0, 2.0) node[midway, above, font=\tiny\bfseries] {1.99$\times$ Speedup};
\end{scope}
...
\begin{scope}[xshift=10.5cm]
    ...
    % Bars
    % PyTorch: 3.67 -> 3.67/2 = 1.835
    \fill[torchgray!80, draw=black] (0.5,0) rectangle (1.5,1.835);
    \node[above, font=\scriptsize\bfseries] at (1.0, 1.835) {3.67 s};
    \node[below, align=center, font=\scriptsize] at (1.0, -0.15) {PyTorch\\\tiny(HuggingFace)};
    
    % Apple MLX: 0.94 -> 0.94/2 = 0.47
    \fill[mlxorange!85, draw=black] (2.5,0) rectangle (3.5,0.47);
    \node[above, font=\scriptsize\bfseries] at (3.0, 0.47) {0.94 s};
    \node[below, align=center, font=\scriptsize] at (3.0, -0.15) {Apple MLX\\\tiny(Metal JIT)};
    
    % VelocityAI: 0.19 -> 0.19/2 = 0.095
    \fill[velblue!90, draw=black, line width=1.1pt] (4.5,0) rectangle (5.5,0.095);
    \node[above, font=\scriptsize\bfseries, text=velblue] at (5.0, 0.095) {\textbf{0.19 s}};
    \node[below, align=center, font=\scriptsize\bfseries, text=velblue] at (5.0, -0.15) {VelocityAI\\\tiny(POSIX mmap)};
    
    % Annotations
    \draw[<->, >=stealth, dashed, thick, velblue] (5.0, 1.2) -- (1.0, 1.2) node[midway, above, font=\tiny\bfseries] {19.3$\times$ Lower Latency};
\end{scope}
\end{tikzpicture}%
}
\caption{Empirical inference performance comparison on Apple Silicon M2 (SmolLM2-360M-Instruct). (a) Autoregressive token decode throughput: VelocityAI achieves 78.41 tok/s on CPU in non-quantized Float16, matching MLX's Metal GPU throughput (77.84 tok/s) and delivering a 1.99$\times$ speedup over PyTorch CPU. (b) Cold-start weight ingestion latency: POSIX zero-copy memory mapping eliminates intermediate heap allocations, yielding a 0.19 s startup time (19.3$\times$ faster than PyTorch and 4.9$\times$ faster than MLX).}
...
\midrule
PyTorch (HF) & FP16 (Non-quant.) & M2 CPU (4 Cores) & $3.67 \pm 0.81$ & 39.34 & $\sim 25.42$ & $\sim 1,850$ MB & $1.00\times$ \\
Ollama (llama.cpp)$^\dagger$ & INT4 (Q4\_K\_M) & M2 CPU (4 Cores) & 0.05$^\dagger$ (Daemon) & 105.29 & $9.50 \pm 0.8$ & $\sim 650$ MB & N/A \\
MLX 0.12.0 & BF16 (Non-quant.) & M2 GPU & $0.94 \pm 0.15$ & 77.84 & $12.85 \pm 0.8$ & $\sim 950$ MB & $1.98\times$ \\
\midrule
\textbf{VelocityAI (Ours)} & \textbf{FP16 (Non-quant.)} & \textbf{M2 CPU (NEON)} & \textbf{0.19} & \textbf{78.41} & $\mathbf{12.75 \pm 1.2}$ & $\mathbf{\sim 145}$ \textbf{MB} & $\mathbf{1.99\times}$ \\
\bottomrule
\end{tabular}%
}
\begin{flushleft}
...
As illustrated in Figure \ref{fig:empirical_benchmarks}(b) and detailed in Table \ref{tab:benchmarks}, VelocityAI achieves a cold-start latency of 0.19 s. This represents a 19.3$\times$ speedup over PyTorch ($3.67 \pm 0.81$ s) and a 4.9$\times$ speedup over Apple MLX (0.94 s). PyTorch's extended delay originates from deserializing tensor dictionaries and allocating dynamic heap memory across individual layers. Apple MLX incurs 0.94 s due to Metal pipeline creation and compute graph compilation. VelocityAI achieves a 0.19 s cold-start by binding the process virtual address space directly to the file descriptor through POSIX \texttt{mmap}, allowing decode execution to commence immediately as pages fault in transparently via the unified memory controller.

\subsection{Autoregressive Decode Throughput (RQ2)}
As shown in Figure \ref{fig:empirical_benchmarks}(a), VelocityAI sustains an average decode speed of 78.41 tokens/second on 4 CPU Performance cores in native Float16 precision. This provides a 1.99$\times$ speedup over PyTorch CPU (39.34 tok/s) and closely matches Apple MLX's dedicated Metal GPU throughput (77.84 tok/s). While Ollama delivers 105.29 tok/s using 4-bit integer quantization (Q4\_K\_M), this throughput comes at the expense of model precision and mathematical degradation, as documented in Section \ref{sec:discussion}. VelocityAI maintains an inter-token latency of $12.75 \pm 1.2$ ms with zero thermal throttling across 10 evaluation trials.
...
\midrule
1 & 196.92 MB & 196.92 MB \\
2 & 393.84 MB & 196.92 MB \\
3 & 590.76 MB & 196.92 MB \\
\bottomrule
\end{tabular}%
}
\begin{flushleft}
\footnotesize $^\dagger$Separate-process RSS is computed theoretically ($N \times 196.92$ MB). VelocityAI shared-memory values were empirically validated on physical hardware via \texttt{ps -o rss} reporting precisely 196.92 MB for 1, 2, and 3-agent combinations.
\end{flushleft}
\end{table}
...
\begin{flushleft}
\footnotesize $^\dagger$Test suite is completely verified via \texttt{benchmark\_cycle\_recovery.py} inside the repository.
\end{flushleft}
\end{table}
...
\section{Conclusion}
VelocityAI demonstrates that hardware-software co-design leveraging zero-copy virtual memory mapping, vectorized ARM NEON Float16 computation, and persistent spin-barrier synchronization achieves 0.19 s cold-start latency and 78.41 tokens/s CPU decoding on Apple Silicon M2, while enabling sequential multi-agent execution with 0 MB of additional model-weight duplication.
